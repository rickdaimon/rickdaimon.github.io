...
$(document).ready(function() {
    ...
    $(document).on("click", "#botFilter .btn", function() {
...
function loadLeft(opts) {
    inlineAlert("", "Loading inventory - please wait...");
    var DIV = "<div class='placeholder matched' data-name='{0}' data-pos='{1}' data-price='{2}' data-bot='{3}'>";
    DIV += "<div class='slot {13}' data-view='{15}' data-name='{4}' data-pos='{5}' data-price='{6}' data-bot='{7}' data-id='{8}''>";
    DIV += "<div class='inspect'>Inspect</div>";
    DIV += "<img src=\"{9}\"></img>";
    ...
    var url = "";
    if (DEPOSIT) {
        url = "/api/get_inv?" + opts;
    } else {
        var g = grecaptcha.getResponse();
        url = "/api/get_bank_safe?g-recaptcha-response=" + g;
    }
    $.ajax({
        "url": url,
        success: function(data) {
            try {
                ...
                if (data.success) {
                    ...
                    $("#availlll").html(formatNum(data.balance));
                    $("#available").html(formatNum(data.available));
                    ...
                    for (var i = 0; i < count; i++) {
                        ...
                        var slot_class = "bounceIn animated";
