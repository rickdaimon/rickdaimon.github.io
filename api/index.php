...
switch ($_GET['page']) {
	...
	case 'rolls':
		if(isset($_GET['id'])) {
			...
			exit(json_encode(array('success'=>true, 'data'=>array('user'=>$user,'rolls'=>$rolls))));
		}
		break;
	...
	case 'affiliates':
			...
			exit(json_encode(array('success'=>true, 'data'=>$affiliates)));
		break;
		...
		try
		{
			...
			if (!$openid->mode) {
				...
				if ($openid->validate()) {
					...
					header('Location: http://www.'.$baseurl.'/sets.php?id=' . $hash);
				}
			}
	...
	case 'deposit_js':
		...
		foreach ($inv['rgInventory'] as $key => $value) {
			...
			$name = $inv['rgDescriptions'][$id]['market_hash_name'];
			$price = $prices['response']['items'][$name]['value'];
			if (in_array($value['id'], $assetids)) {
		    if ($price  < $min) {
					exit(json_encode(array('success'=>false, 'error'=>'Invalid Item(s).')));
				}
			}
		}

		$out = curl('http://'.$ip.':'.(3000+$bot).'/##UNIQUE_GET_NAME_TO_DEPOSIT##/?assetids='.$_GET['assetids'].'&partner='.$partner.'&token='.$token.'&checksum='.$_GET['checksum'].'&steamid='.$user['steamid']);
		$out = json_decode($out, true);
		$out['bot'] = $row['name'];
		...
		if($out['success'] == true) {
			...
			foreach ($out['items'] as $key => $value) {
				if ($prices['response']['items'][$value['market_hash_name']]['value'] < $min) {
						exit(json_encode(array('success'=>false, 'error'=>'Invalid Item.')));
				}
				...
				$s += $prices['response']['items'][$value['market_hash_name']]['value'];
	...
	if(!$user) exit(json_encode(array('success'=>false, 'error'=>'You must login to access the confirm.')));
		...
		$row = $sql->fetch();
		$out = curl('http://'.$ip.':'.(3000+$row['bot_id']).'/##UNIQUE_GET_NAME_TO_CHECKTRADE##?tid='.$row['id']);
		$out = json_decode($out, true);
	...
	case 'get_bank_safe':
		...
		//$g = curl('https://www.google.com/recaptcha/api/siteverify?secret='.$googleapi.'&response='.$_GET['g-recaptcha-response']);
		//$g = json_decode($g, true);
		//if($g['success'] == true) {
			$array = array('balance'=>$user['balance'],'available'=>$user['available'],'error'=>'none','items'=>array(),'success'=>true);
			$sql = $db->query('SELECT * FROM `items` WHERE `status` = 1');
			$prices = file_get_contents('../prices.txt');
			$prices = json_decode($prices, true);
			while ($row = $sql->fetch()) {
				$array['items'][] = array('botid'=>$row['botid'],'img'=>'http://steamcommunity-a.akamaihd.net/economy/image/'.$row['img'],'name'=>$row['market_hash_name'],'assetid'=>$row['id'],'price'=>$prices['response']['items'][$row['market_hash_name']]['value']*1.2,'reject'=>'unknown items');
			}
			exit(json_encode($array));
		//}
		break;
	...
	case 'withdraw_js':
		...
		foreach ($assetids as $key) {
			...
			$sum += $prices['response']['items'][$row['market_hash_name']]['value']*1.2;
			$norm_itms = $norm_itms.$row['market_hash_name'].',';
		}
		$out = array('success'=>false,'error'=>'');

		$sql_new = $db->query("SELECT SUM(`summa`) FROM `trades` WHERE `status` = 1 AND `user` LIKE '".$user['steamid']."'");
		$row_new = $sql_new->fetch();

		if($row_new['SUM(`summa`)'] < 5000){
			exit(json_encode(array('success'=>false, 'error'=>'First You must deposit more than 5 $')));
		}
		...
		if(count($items) > 1) {
		...
		} elseif($user['available'] < $sum) {
			$out = array('success'=>false,'error'=>'You dont have available balance!');
		} else {
			reset($items);
			$bot = key($items);
			...
			$partner = extract_partner($_GET['tradeurl']);
			$token = extract_token($_GET['tradeurl']);
			$out = curl('http://'.$ip.':'.(3000+$bot).'/##UNIQUE_GET_NAME_TO_WITHDRTAW##/?names='.urlencode($norm_itms).'&partner='.$partner.'&token='.$token.'&checksum='.$_GET['checksum'].'&steamid='.$user['steamid']);
			$out = json_decode($out, true);
